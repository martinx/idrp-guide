...
- [ ] 每一步的停留时长观感是否合理（这一步理论上应该被 06 章的时长计算保证，但首次跑通时人工确认一遍，能及早发现 Spec 里 `min_hold_sec` 设置不合理的情况）。
...
演示账号（10.2 节第一条）除了数据要脱敏，权限范围也应该被限制到"仅能访问被录制的这个功能所需的最小权限"，理由有二：一是避免录制脚本因为某个误操作（比如 04 章清洗后的脚本因选择器变化点错了元素）意外触达高权限操作产生真实副作用；二是即使录制素材后续意外泄露，暴露的账号权限影响面也是可控的。这一条原则同样适用于 09 章编排器里 `preflightCheck` 使用的账号——preflight 会话和正式录制会话应该用同一个受限的演示账号，不应该为了图方便临时切换成权限更高的账号去"确保脚本能跑通"。
