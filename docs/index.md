...
如果你只能读一部分，建议至少通读第 00 章和第 16 章。一条视频从发起到成稿真实耗时约
15～30 分钟，人工唯一不可省略的环节是最后的审片。

## 全书覆盖的能力

| 能力 | 说明 | 章节 |
|---|---|---|
| 无人值守录制引擎 | ready/go 握手信号杜绝录到不该出现的画面、多屏环境自动探测录制设备、浏览器换皮去掉测试工具身份 | [05](05-recording.md) |
| 字幕双来源判定 | 优先用自动化脚本自带的时间点文本（最准），没有就用本地语音识别（Whisper/SenseVoice）转写兜底 | [06](06-dubbing.md) |
| 自然语速配音 | 按字幕分段生成配音，不做时长强制拉伸，用累计时间戳自然吸收每句的细微误差 | [06](06-dubbing.md) |
| 多音字修复表 | 一次维护"问题词→安全替换词"，比如"命令行工具"读错音，改一次对全部功能点视频生效 | [06](06-dubbing.md) |
| 配音全局偏移 | 一个 `dub_offset` 就能整体微调配音快慢，配合 tpad 兜底，正偏移也不会把配音截断 | [08](08-mixing.md) |
| 多语言一键翻译 | DeepSeek 批量翻译整份字幕，操作录制完全不用为了出英文版重录第二遍 | [06](06-dubbing.md) |
| 文件即状态的编排 | 没有数据库、没有状态机，几个约定命名的文件就是整条流水线的进度，任何一步都能单独重跑 | [09](09-orchestrator.md) |
| meta.json 三级配置 | 内置默认 → 项目级 → 功能级层层覆盖，团队规范改一处，全部功能点视频统一生效 | [09](09-orchestrator.md) |
| AI 零审阅生成脚本 | `record.spec.js` 默认由 AI 直接产出，人工 codegen 只是 AI 猜不准页面结构时的兜底路径 | [03](03-feature-spec.md) |
| AI 自愈调试循环 | 冒烟测试报错直接交给 AI 分析、修复选择器、重新验证，人不用打开控制台排查 | [17](17-ai-spec-and-codegen.md) |
| AI 自动化质检 | 抽帧 + 多模态模型自动比对防泄露清单，产出审片报告，人只看标红的风险点 | [19](19-ai-qa-leak-detection.md) |
| 防泄露检查清单 | 10 类真实翻车场景（菜单栏组件、终端历史、访问限制页返回200……）总结成可执行清单 | [10](10-quality-checklist.md) |
